...
def run_training():
    ...
    if args.num_class == 2:   
        idx = [i for i in range(len(train_dataset)) if train_dataset.imgs[i][1] != 2]
    ...
    if args.num_class == 2:   
        idx = [i for i in range(len(val_dataset)) if val_dataset.imgs[i][1] != 2]
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        for (imgs, targets) in train_loader:
            iter_cnt += 1
            optimizer.zero_grad()
            imgs = imgs.to(device) # 8, 3, 112, 112
