...
def run_training():
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        acc = float(correct_sum) / len(new_train_loader)
        running_loss /= iter_cnt
        train_accs.append(np.around(np.array(acc), 4))
        ...
        with torch.no_grad():
            ...
            acc = float(bingo_cnt) / len(new_val_loader)
            ...
            torch.save({'iter': epoch,
                           ...
                           os.path.join(checkpoint_dir, f"video_classification_epoch{epoch}_acc{acc:.4f}.pth"))
