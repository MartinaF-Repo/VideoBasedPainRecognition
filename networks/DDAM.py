...
class DDAMNet(nn.Module):
    def __init__(self, num_class=2, num_head=2, pretrained=True):
        ...
        # Congelare i parametri della parte pre-addestrata
        ...
        self.fc = nn.Linear(512, num_class) # fully connected
        self.bn = nn.BatchNorm1d(num_class)
        
    def forward(self, x):
        x = self.features(x) #torch.Size([8, 3, 112, 112])
        heads = []
        ...
        y = x*y #[8, 512, 7, 7]
        y = self.Linear(y) #[8, 512, 1, 1]
        y = self.flatten(y) #[8, 512]
        out = self.fc(y) #[8, 2]       
