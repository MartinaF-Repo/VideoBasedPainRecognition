...
class CoordAtt(nn.Module):
    ...
    def forward(self, x):
        identity = x # torch.Size([8, 128, 28, 28])
        n,c,h,w = x.size()
        x_h = self.pool_h(x) #torch.Size([8, 128, 28, 1])
        x_w = self.pool_w(x).permute(0, 1, 3, 2) #torch.Size([8, 128, 28, 1])
        
        y = torch.cat([x_h, x_w], dim=2) # torch.Size([8, 128, 56, 1])
        y = self.conv1(y)
        y = self.bn1(y)
        y = self.relu(y) 
