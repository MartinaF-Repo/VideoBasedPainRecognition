...
def sharpen_transform_opencv(img):
    return sharpen_image_opencv(img)
...
def run_training():
    ...
    data_transforms = transforms.Compose([
        ...
        transforms.Lambda(sharpen_transform_opencv),
        transforms.ToTensor(),
    ...
    new_trainloader = video_loader(train_loader)
    random.shuffle(new_trainloader)
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        for (videos, targets) in new_trainloader:
            iter_cnt += 1
            optimizer.zero_grad()
        ...
        acc = correct_sum.float() / (len(train_dataset) / 8)
        ...
        with torch.no_grad():
            ...
            for videos, targets in new_valloader:
                ...
                videos = videos.permute(1,2,0,3,4).to(device)
                targets = targets.to(device)
                out, feat, heads = model(videos)
            ...
            checkpoint_dir = './checkpoints_DDAM3D'
